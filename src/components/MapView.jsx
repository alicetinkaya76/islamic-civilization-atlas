...
export default function MapView({ lang, t }) {
  ...
  useEffect(() => {
    ...
    if (layers.dynasties) {
      DB.dynasties.forEach(d => {
        ...
        L.rectangle([[d.bs, d.bw], [d.bn, d.be]], {
          ...
          `<div class="p-title">${n(d, lang)}</div>` +
    ...
    if (layers.battles) {
      DB.battles.forEach(b => {
        ...
        L.marker([b.lat, b.lon], { icon }).bindPopup(
          `<div class="p-title">⚔ ${n(b, lang)}</div>` +
    ...
    if (layers.events) {
      DB.events.forEach(e => {
        ...
        L.marker([e.lat, e.lon], { icon }).bindPopup(
          `<div class="p-title">📜 ${n(e, lang)}</div>` +
          ...
          (e.desc ? `<div class="p-desc">${e.desc}</div>` : ''),
          { maxWidth: 320 }
        ).addTo(lg.events);
      });
    }
    ...
    if (layers.scholars) {
      DB.scholars.forEach(s => {
        ...
        L.circleMarker([s.lat, s.lon], {
          ...
          `<div class="p-title">📚 ${n(s, lang)}</div>` +
    ...
    if (layers.monuments) {
      DB.monuments.forEach(m => {
        ...
        L.marker([m.lat, m.lon], { icon }).bindPopup(
          `<div class="p-title">🕌 ${n(m, lang)}</div>` +
    ...
    if (layers.cities) {
      ...
      Object.values(best).forEach(c => {
        ...
        L.circleMarker([c.lat, c.lon], {
          ...
          `<div class="p-title">🏙 ${n(c, lang)}</div>` +
  ...
  }, [year, layers, filters, lang, t, analyticsMap]);
